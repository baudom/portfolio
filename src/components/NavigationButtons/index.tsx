      ...
      (entries) => {
        entries.forEach((entry) => {
          if (`#${entry.target.id}`.endsWith(ANCHOR_START)) {
            setNavShown(entry.intersectionRatio < VISIBILITY_THRESHOLD);
      ...
      <div className="flex flex-row lg:flex-col gap-12 lg:gap-4">
        {quickLinks.map((link, idx) => (
          ...
          >
            <div
              className={
                `flex items-center justify-center w-8 lg:w-12 h-8 lg:h-12 rounded-xl transition-all duration-300 ease-in-out group-hover:shadow-lg group-hover:scale-105 ` +
                (activeIdx === idx
            ...
            <span
              className={`text-xs lg:hidden font-bold ${
                activeIdx === idx ? "text-secondary" : "text-muted"
              }`}
            >
              {link.title}
            </span>
